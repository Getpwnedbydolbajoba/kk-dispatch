...
RegisterKeyMapping(Config.CommandName, 'Open dispatch', 'keyboard', Config.KeyBind)
...
CreateThread(function()
    while true do
        if not warnedshooting and IsPedShooting(cache.ped) and not IsPedCurrentWeaponSilenced(cache.ped) then
            if ESX.PlayerData.job.name == Config.Jobname then return end
            TriggerEvent('kk-dispatch:client:sendDispatch', '10-71', Config.Jobname, 'SHOOTING')
