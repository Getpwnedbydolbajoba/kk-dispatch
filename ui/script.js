$(document).ready(function() {
    ...
    window.addEventListener('message', function (event) {
        ...
        if (event.action === 'addCall') {
            ...
            $.post("https://kk-dispatch/loadCalls", JSON.stringify({})); menuOpen = true 
        } else if (event.action === 'loadCalls') {
            Functions.LoadCalls(event.data)
        } else if (event.action === 'closeDispatch') {
            $.post("https://kk-dispatch/closeDispatch", JSON.stringify({})); menuOpen = false
    ...
    Function.AcceptCall = function(id) {
        $.post("https://kk-dispatch/acceptCall", JSON.stringify({id: id}));
    }

    Functions.CreateNotification = function(data) {
        if (menuOpen) {
            $.post("https://kk-dispatch/loadCalls", JSON.stringify({}));
        } else {
            let $notification
